// Package config reads agentsync's configuration from the environment.
package config
...
func Load(getenv func(string) string) (Config, error) {
	...
	cfg := Config{
		DB:               db,
		Listen:           get("AGENTSYNC_LISTEN", ":8091"),
		AgentDownloadURL: get("AGENTSYNC_AGENT_DOWNLOAD_URL", "https://github.com/jyothri/bhandaar/releases/latest"),
	}

	secret := getenv("AGENTSYNC_JWT_SECRET")
	if secret == "" {
		errs = append(errs, errors.New("AGENTSYNC_JWT_SECRET is required (generate one with: openssl rand -base64 48)"))
	} else if b, err := base64.StdEncoding.DecodeString(strings.TrimSpace(secret)); err != nil {
		errs = append(errs, fmt.Errorf("AGENTSYNC_JWT_SECRET: not valid base64: %v", err))
	} else if len(b) < MinJWTSecretBytes {
		errs = append(errs, fmt.Errorf("AGENTSYNC_JWT_SECRET: %d bytes after decoding, need at least %d", len(b), MinJWTSecretBytes))
	} else {
		cfg.JWTSecret = b
	}
	...
	cfg.AccessTTL = duration("AGENTSYNC_ACCESS_TTL", "15m")
	cfg.RefreshTTL = duration("AGENTSYNC_REFRESH_TTL", "720h")
	...
	cfg.MinAgentVersion = version("AGENTSYNC_MIN_AGENT_VERSION")
	cfg.LatestAgentVersion = version("AGENTSYNC_LATEST_AGENT_VERSION")
	if cfg.LatestAgentVersion.Less(cfg.MinAgentVersion) {
		errs = append(errs, fmt.Errorf("AGENTSYNC_LATEST_AGENT_VERSION (%s) is below AGENTSYNC_MIN_AGENT_VERSION (%s)",
			cfg.LatestAgentVersion, cfg.MinAgentVersion))
	}

	for _, s := range strings.Split(get("AGENTSYNC_TRUSTED_PROXIES", defaultTrustedProxies), ",") {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("AGENTSYNC_TRUSTED_PROXIES: %v", err))
			continue
		}
