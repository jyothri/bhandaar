...
|---|---|---|
| `agentsync-docker-image.yml` | The `agentsync` server image | Docker Hub `jyothri/bhandaar-agentsync`, on merge to `main`, exactly like `be` and `ui` |
...
## Server: `agentsync-docker-image.yml`

This mirrors `backend-docker-image.yml`.

```yaml
name: Docker agentsync Image CI

on:
  push:
    branches: ["main"]
    paths:
      - "agentsync/**"            # includes agentsync/wire
      - ".github/workflows/agentsync-docker-image.yml"
  pull_request:
    branches: ["main"]
    paths:
      - "agentsync/**"
...
jobs:
  test:
    ...
    env:
      AGENTSYNC_TEST_DB: postgres://postgres:postgres@localhost:5432/postgres?sslmode=disable
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-go@v5
        with:
          go-version-file: agentsync/go.mod
          cache-dependency-path: agentsync/go.sum
      - name: Test wire module (standard library only)
        run: go test -race ./...
        working-directory: agentsync/wire
  ...
  agentsync:
    ...
    steps:
      ...
      - name: Lowercase the repo name and username
        run: echo "REPO=${GITHUB_REPOSITORY,,}-agentsync" >>${GITHUB_ENV}
...
- `wire` is a separate module (see [Shared wire module](remote-sync-server.md#shared-wire-module)), so `go test ./...` in `agentsync/` doesn't reach it; it gets its own step.
- The image name follows the existing pattern: `${repo}` for `be`, `${repo}-ui` for `ui`, and `${repo}-agentsync` here, giving `jyothri/bhandaar-agentsync`. Tags are the same as for `be`/`ui`: `:latest` plus the action's default per-commit tag. Prod runs `:latest`.
...
The base image is Alpine, like `be`'s, so a shell is available for debugging. The binary is on `PATH` and runs as a non-root user. That makes the admin commands work as `docker exec -it <container> agentsync user add …`. The Go base-image tag is pinned to the `agentsync/go.mod` toolchain, not the floating `golang:alpine` that `be` uses, so the image is built with the Go version CI tested.

### Deploying

Unchanged from how `be`/`ui` are deployed today. After the image is pushed, the user pulls `jyothri/bhandaar-agentsync:latest` on the prod box and restarts the `agentsync` compose service. The compose and nginx changes are in the [server spec](remote-sync-server.md#deployment).

## Agent: `driveagent.yml`

### Versioning

- **The version is a constant**, `Version` in `agent/linux/internal/version/version.go` (`const Version = "0.2.0"`), bumped by hand in the PR that changes the agent. Local `go build`s therefore report a real version, so the server's minimum-version check treats a dev build like the matching release.
...
The module lives in `agent/linux/` but now also ships for macOS. The directory isn't renamed as part of this work. The macOS specifics are under [Platforms](remote-sync-agent.md#platforms) in the agent spec.
...
jobs:
  test:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version-file: agent/linux/go.mod
          cache-dependency-path: agent/linux/go.sum
      - run: go vet ./...
        working-directory: agent/linux
  ...
  version-check:
    ...
    steps:
      ...
      - id: v
        run: ./agent/linux/scripts/version-check.sh   # see "version-check" below
        env:
      ...
      - name: Build
        ...
        run: |
          ...
          GOOS=$os GOARCH=$arch CGO_ENABLED=0 go build -trimpath \
            -ldflags "-s -w -X github.com/jyothri/bhandaar/agent/linux/internal/version.Commit=${GITHUB_SHA::7}" \
    ...
    steps:
      ...
      - name: Create tag and GitHub Release
        ...
        run: |
          ...
          notes=$(git log --no-merges --format='- %s (%h)' ${prev:+$prev..}HEAD -- agent/linux agentsync/wire)
...
This is a small script, `agent/linux/scripts/version-check.sh`, so it can also be run locally. It reads `Version` from `internal/version/version.go`, checks it's valid semver (`MAJOR.MINOR.PATCH`), and finds `latest` = the highest existing `driveagent/v*` tag (`git tag --sort=-v:refname`). Then:
...
- The server's `AGENTSYNC_AGENT_DOWNLOAD_URL` defaults to `https://github.com/jyothri/bhandaar/releases/latest`, which the handshake returns with `upgrade_recommended` and `upgrade_required`.
- `AGENTSYNC_LATEST_AGENT_VERSION` and `AGENTSYNC_MIN_AGENT_VERSION` are set by hand in the prod `.env` after an agent release. Raise `LATEST` for every release, so older agents get a nudge. Raise `MIN` only when a release is required, for example for a protocol change. Automating this (the server reading the latest release from the GitHub API) is possible later, but not part of v1.

## Rollout placement

- `agentsync-docker-image.yml` and the Dockerfile ship with rollout step 1 (server skeleton).
