...
- Order: the drive checks come first, before anything is walked: the `--drive-root` check (and, with `--replace-root`, clearing the drive's old data), then recording the drive (`scan.Prepare`). Only then is the folder walked and hashed (`scan.Run`).
