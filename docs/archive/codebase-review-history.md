# Codebase Review: UI, Backend, Ops — History

Archived on 2026-09-24. This file keeps every **done** and **won't-fix** item from the review, verbatim and under its original number, with the change log and the investigation notes. Open and pending items, including new ones split out of the notes below, are in [../codebase-review.md](../codebase-review.md); each open item here is a one-line pointer to it. The local development setup moved to [../local-dev.md](../local-dev.md).
...
*2026-09-24:* 53 done, 1 won't fix (7.9), 6 open (3.7, 4.1, 4.2, 5.1, 6.3, 7.6 deferred). Open items continue in [../codebase-review.md](../codebase-review.md).
...
- [ ] **5.1** Delete or use the unused types in `src/types/optionals.ts` — *open; see [../codebase-review.md](../codebase-review.md)*
...
As it stood when the review was archived. The current order is in [../codebase-review.md](../codebase-review.md).
...
- [ ] **6.3 TypeScript 7** — *open; see [../codebase-review.md](../codebase-review.md)*
...
- [x] **7.3 The final redirect goes wherever the caller says** — `be/web/oauth.go:28`, `:110-121`
  ...
  *Done in #17:* `redirectUri` must have the same origin (scheme, host, port, and no userinfo) as an origin in `-frontend_url`. It's checked before the code is exchanged, and the redirect target is built from the flag, never from the request.
  ...
  - *Update (2026-09-24, after deploying #17):* production uses only `FRONTEND_URL=https://sm.jkurapati.com`. The LAN address isn't used to reach production, so it's left out on purpose.
...
- [ ] **7.6 Progress events never carry `completion_pct` or `eta_in_sec`** — *open; see [../codebase-review.md](../codebase-review.md)*
