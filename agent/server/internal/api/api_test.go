...
func testConfig(t *testing.T, env map[string]string) config.Config {
	t.Helper()
	all := map[string]string{"AGENTSERVER_JWT_SECRET": base64.StdEncoding.EncodeToString(bytes.Repeat([]byte{7}, 32))}
	for k, v := range env {
		all[k] = v
	}
...
func TestHealth(t *testing.T) {
	...
	for _, c := range []struct {
		...
		if h.Status != c.want || h.Service != "agentserver" || len(h.APIVersions) != 1 || h.APIVersions[0] != "v1" {
			t.Errorf("health = %+v", h)
		}
...
func TestClientIP(t *testing.T) {
	srv := New(testConfig(t, map[string]string{"AGENTSERVER_TRUSTED_PROXIES": "192.168.1.118"}), nil, nil, fakeDB{})
	var got string
