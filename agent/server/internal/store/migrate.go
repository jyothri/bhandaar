...
const migrationLockID = 0x61676e7473727672 // "agntsrvr"
...
// agentserver_schema_migrations yet, in version order, each in its own
...
func Migrate(ctx context.Context, pool *pgxpool.Pool, dir fs.FS) error {
	...
	for _, m := range migs {
		...
		err := pgx.BeginFunc(ctx, conn, func(tx pgx.Tx) error {
			...
			_, err := tx.Exec(ctx,
				"INSERT INTO agentserver_schema_migrations (version, applied_at) VALUES ($1, now())", m.version)
			return err
		})
		if err != nil {
...
func appliedVersions(ctx context.Context, conn *pgx.Conn) (map[int]bool, error) {
	...
	if err := conn.QueryRow(ctx,
		"SELECT to_regclass('agentserver_schema_migrations') IS NOT NULL").Scan(&exists); err != nil {
		return nil, err
	}
	...
	rows, err := conn.Query(ctx, "SELECT version FROM agentserver_schema_migrations")
	if err != nil {
		return nil, err
	}
