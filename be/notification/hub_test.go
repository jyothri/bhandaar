...
// it, so its broadcasts can't leak into later tests' SubscribeAll()
// subscribers.
...
func TestEverySubscriberReceivesEveryUpdate(t *testing.T) {
	a, unsubA := SubscribeAll()
	defer unsubA()
	b, unsubB := SubscribeAll()
	defer unsubB()
...
func TestFullBufferKeepsNewestUpdate(t *testing.T) {
	...
	// Subscribe to this publisher's own key (not SubscribeAll(), which other
...
func TestPublisherCloseClosesOnlyItsOwnSubscribers(t *testing.T) {
	...
	case _, ok := <-all:
		if !ok {
			t.Fatal("all-scans subscriber was closed by a scan's publisher")
		}
...
// Review follow-up: a scan whose client key happens to be "all" used to be
// broadcast twice to all-scans subscribers, and its cleanup closed every SSE
// client's channel. All-scans subscribers now live in their own set.
func TestClientKeyAllDoesNotCollideWithSubscribeAll(t *testing.T) {
	all, unsub := SubscribeAll()
	defer unsub()

	pub := GetPublisher("all")
	publish(t, pub, Progress{ScanId: 1})
	closeAndWait(t, pub, "all")

	if got := receive(t, all).ScanId; got != 1 {
		t.Fatalf("got scan %d, want 1", got)
	}
	select {
	case p, ok := <-all:
		if !ok {
			t.Fatal("all-scans subscriber was closed when the \"all\" scan ended")
		}
		t.Fatalf("update delivered twice (second: %+v)", p)
	case <-time.After(50 * time.Millisecond):
	}
}
