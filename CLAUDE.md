...
- **Docs**: `docs/codebase-review.md` lists open review items only; completed and won't-fix items, with the change log, are in `docs/archive/codebase-review-history.md` under the same numbers. When an item is done, move it to the archive and add a change-log row there. Local machine setup is in `docs/local-dev.md`. Design specs live in `docs/specs/` (currently the drive comparison agent in `agent/linux/`); their design history is in `docs/archive/`. `docs/archive/be/` holds the backend's earlier issue plans and status notes (December 2025), kept as written: paths inside them still say `be/docs/`.
