...
- **Agent sync (agentsync/)**: Go service that receives `driveagent` scan uploads (health, handshake, login/refresh so far; see `docs/specs/remote-sync-server.md`). Its request/response types are in `agentsync/wire`, a separate standard-library-only module
...
Navigate to `agentsync/` for these. Go isn't installed on the dev box, so run them in the container matching `agentsync/go.mod`:

```bash
# Tests; the Postgres ones skip unless AGENTSYNC_TEST_DB is set (each test gets its own schema)
docker run --rm --network host -v "$PWD":/src -w /src \
  -e AGENTSYNC_TEST_DB='postgres://postgres:postgres@localhost:5432/agentsync_test?sslmode=disable' \
  golang:1.27.1 sh -c 'go test -race ./... && cd wire && go test -race ./...'

# Image (from the repository root)
docker build . -f agentsync/build/Dockerfile -t jyothri/bhandaar-agentsync

# Admin, inside the container: add a user (prompts for the password), list, run housekeeping once
agentsync user add --username NAME
agentsync user list
agentsync housekeeping
```

`agentsync serve` needs `AGENTSYNC_JWT_SECRET` (`openssl rand -base64 48`) and the same `DB_*` variables as the backend; it listens on `:8091`. CI (`agentsync-docker-image.yml`) tests against a Postgres service container and pushes `jyothri/bhandaar-agentsync` on merge to `main`.
